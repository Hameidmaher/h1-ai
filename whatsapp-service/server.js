    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            if (msg.message.buttonsResponseMessage) {
                ...
                text = msg.message.templateButtonReplyMessage.selectedDisplayText || '';
                buttonId = msg.message.templateButtonReplyMessage.selectedId || '';
                isButtonReply = true;
            } else if (msg.message.listResponseMessage) {
                const single = msg.message.listResponseMessage.singleSelectReply;
                if (single) {
                    buttonId = single.selectedRowId || '';
            ...
            try {
                ...
                const response = await axios.post(
                    `${BACKEND_URL}/v1/whatsapp/incoming`,
                    {
                        ...
                        pharmacy_id: sessions[phoneNumber]?.pharmacy_id || null,
                ...
                console.log(`📤 [${phoneNumber}] Replied: ${reply.substring(0, 100)}`);
...
app.post('/send-buttons', async (req, res) => {
    const { phone_number, to, text, buttons, footer } = req.body;
    ...
    if (!buttons || buttons.length === 0) {
        return res.status(400).json({ error: 'buttons required' });
    }
    ...
    try {
        ...
        try {
            ...
            console.log(`⚠️ Modern failed: ${e1.message}, trying legacy...`);
            try {
                result = await session.sock.sendMessage(jid, {
                    text: text,
                    footer: footer || '',
                ...
                console.log(`✅ Legacy buttons sent to ${to}`);
            } catch (e2) {
                // Fallback: نص عادي
                const numbered = text + '\n\n' + buttons.map((b,i) => `${i+1}. ${b.title}`).join('\n');
                result = await session.sock.sendMessage(jid, { text: numbered });
                console.log(`✅ Sent as text to ${to}`);
            }
        }
