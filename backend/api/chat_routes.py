...
async def log_audit(
    ...
    """تسجيل عملية في audit_logs (non-blocking)."""
    try:
        s = SessionLocal()
        # ✅ تأكد من UUID صالح أو استبدله بقيمة آمنة
        import uuid
        try:
            uuid.UUID(str(user_id))
        except (ValueError, AttributeError):
            # لو مش UUID، نستخدم UUID وهمي (لأن العمود من نوع varchar)
        ...
        s.execute(text("""
            ...
            VALUES 
                (:uid, :un, :act, :ent, :eid, :det, :ip, :ua)
        """), {
            "uid": str(user_id),
            "un": (username or "anonymous")[:50],
            "act": action[:50],
            "ent": entity[:50],
            "eid": (entity_id or "")[:100],
            "det": (details or "")[:500],
            "ip": (ip_address or "0.0.0.0")[:45],
        ...
        import structlog
        structlog.get_logger().warning("audit.log_failed", error=str(e)[:150])
...
async def send_message(req: ChatMessageRequest):
    ...
    try:
        ...
        # ✅ Audit logging
        await log_audit(
            user_id=req.customer_id or "00000000-0000-0000-0000-000000000000",
            username=req.channel_user_id[:50],
            action="chat_message",
            entity="chatbot_session",
            entity_id=response.session_id,
            details=f"len={len(req.message)}, tools={len(response.tool_calls)}",
        )
        ...
        logger.error("chat.api_failed", error=str(e)[:300], exc_info=True)
        raise HTTPException(
